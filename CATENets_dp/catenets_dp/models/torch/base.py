...
#import catenets_dp.logger as log
...
from .utils.decorators import benchmark, check_input_train
from .utils.model_utils import make_val_split
...
class BasicNet(nn.Module):
    ...
    def fit(
        self, X: torch.Tensor, y: torch.Tensor, epsilon = 1, delta = 1e-5, weight: Optional[torch.Tensor] = None
    ) -> "BasicNet":
        self.train()
        ...
        training_dataset = CustomDataset(X, y)
        data_loader = torch.utils.data.DataLoader(training_dataset, batch_size = batch_size)
